...
def updateHandlers(client, message,redis):
		...
		if text and group == False:
			if (rank == "sudo" or rank == "sudos" or rank == "asudo") or (redis.get("{}Nbot:autoaddbot".format(BOT_ID)) and GPranks(userID,chatID) == "creator"):
				...
				if text == c.add and not redis.sismember("{}Nbot:disabledgroups".format(BOT_ID),chatID) and Ckuser(message):
					...
					kb = InlineKeyboardMarkup([[InlineKeyboardButton(r.MoreInfo, url="t.me/Source_Ze")]])
					...
					get = (redis.hget("{}Nbot:links".format(BOT_ID),chatID) or GetLink(chatID) or "https://t.me/Source_Ze")
	...
	if type == enums.ChatType.PRIVATE and message.outgoing != True:
		...
		if text and re.search("^/start$",text):
			...
			kb = InlineKeyboardMarkup([[InlineKeyboardButton("🔱 𝐒𝐎𝐔𝐑𝐂𝐄 𝐙𝐄 🔱", url="t.me/Source_Ze")]])
